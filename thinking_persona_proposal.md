...
| E4 persona space | Hollow (CoT) role cloud overlaps filled (answer) cloud | Hollow cloud collapses to a blob at the default-CoT point (roles don't move it) | Hollow cloud same shape, translated as a block |
...
- Metric: cosine between the **mean-centered** residual-stream activation and the axis direction. Raw activations share a large mean component that dominates any cosine; subtract the repo's mean activation at that layer (or the mean over default no-think answer tokens, fixed once) before projecting. Also store raw dot product and activation norm for the norm-confound check.
- Do not store activations (200 prompts x 2k tokens x 5k dims ~ 8GB/layer). Compute scalar projections inside the hook, for all directions and (if sweeping) all layers in one pass, and store only scalars.
...
- A null direction with realistic structure: difference-of-means between two random halves of the default-prompt activations.
- One random unit vector (fixed seed). Note: in ~5k dims a random direction has ~0 cosine with everything, so its gap is trivially ~0; report it but do not lean on it. The orthogonalized role vectors, the null direction, and answer-space PCs 2–5 are the real specificity controls.
...
token_idx (absolute position, log it: natural CoT starts ~50, natural answers ~1000, and position alone may shift projection),
is_think (bool), pos_from_boundary (int, negative inside think),
...
8. `cot_in_special` / `answer_in_special`: same text inside another *special-token* block the chat template supports (e.g. `<tool_response>...</tool_response>`). `<think>`/`</think>` are special tokens in Qwen3; `<scratch>` is ordinary text, so `<scratch>` alone confounds "think tag" with "special token". Include both controls and say so.

Because attention is causal, tokens after the measured span cannot affect it: each condition only needs a correct **prefix** (template + tags + transplanted text). No need to invent a following answer for `answer_as_cot`; `cot_as_answer` just uses the empty think block the no-think template already emits.

Teacher-forced forward pass, measure mean cosine on the transplanted span. Plot 2: bar per condition with CIs. Decision rule: does projection track the tag or the content? Does `<scratch>` / `<tool_response>` behave like `<think>` (any tag / any special block) or like plain answer (think-specific)? Also compare early-CoT vs late-CoT in E1 to see whether absolute position alone moves projection.
...
- Judge: a cheap API model with a fixed prompt, sentence-level labels ∈ {in-character first person, meta-stance ("the user wants me to be X", "as an AI I should…"), neutral, refusal/breaking character}. Hand-label 30 sentences first and report agreement. Exclude or flag refusals/broken-character samples (they give large answer shifts for uninteresting reasons). Color scatter points by meta-stance rate. Also run the judge on the role-less E1 CoTs by domain: that is a black-box persona measure independent of the geometry, and belongs next to Plot 1. Plot 13 (optional): meta-stance fraction vs normalized CoT position.
...
- Unit of analysis is the prompt (E1, E2) or the role (E3, E4). Never the token. Paired bootstrap over prompts; report the standardized gap with a 95% CI. No p-values across a layer sweep.
- Report the `</think>` token and the first 5 answer tokens as a separate row in every table; exclude them from answer means if they are outliers, and say so.
...
| 1–2 | vLLM generation, **advice domain first** (E2 depends on it), then the other 3 domains × think + no-think + step-by-step; role prompts (start in background) |
...
Keep a running `results.md` updated every hour (figures + one-line claims) so the write-up starts at hour 9 with material in place.
